...
public class HouseAffordabilityHelper {
    ...
    public static boolean validateAffordability(String houseIncome, String loanTerm, String interestRate, String debtPayBack, String downPayment, String propertyTax, String hoaFee, String insurance){
        if(houseIncome.isEmpty() || loanTerm.isEmpty() || interestRate.isEmpty() || debtPayBack.isEmpty() || downPayment.isEmpty() || propertyTax.isEmpty() || hoaFee.isEmpty() || insurance.isEmpty()){
            JOptionPane.showMessageDialog(null, "Make sure no inputs are empty!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if(houseIncome.matches("\\.+") || loanTerm.matches("\\.+") || interestRate.matches("\\.+") || debtPayBack.matches("\\.+") || downPayment.matches("\\.+") || propertyTax.matches("\\.+") || hoaFee.matches("\\.+") || insurance.matches("\\.+")){
            JOptionPane.showMessageDialog(null, "Make sure no inputs contain only a decimal!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        double houseIncomeNum = Double.parseDouble(houseIncome);
        double interestRateNum = Double.parseDouble(interestRate);
        double debtPayBackNum = Double.parseDouble(debtPayBack);
        double downPaymentNum = Double.parseDouble(downPayment);
        double propertyTaxNum = Double.parseDouble(propertyTax);
        double hoaFeeNum = Double.parseDouble(hoaFee);
        double insuranceNum = Double.parseDouble(insurance);
        int loanTermNum = Integer.parseInt(loanTerm);
        
        if(houseIncomeNum == 0 && interestRateNum == 0 && debtPayBackNum == 0 && downPaymentNum == 0 && propertyTaxNum == 0 && hoaFeeNum == 0 && insuranceNum == 0 && loanTermNum == 0){
            JOptionPane.showMessageDialog(null, "All inputs can not be zero!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if(loanTermNum == 0){
            JOptionPane.showMessageDialog(null, "Loan term can not be 0!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if(downPaymentNum >= 100){
            JOptionPane.showMessageDialog(null, "Down payment can not be great than or equall to 100%!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        
        if(debtPayBackNum >= houseIncomeNum){
            JOptionPane.showMessageDialog(null, "Monthyl debt payback can not be greater than or equal to household income!", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return false;
        }
    ...
    public static double[] calculateLoan(double annualIncome, int loanTerm, double interestRate,
                                                              double monthlyDebtPayments, double downPayment,
                                                              double propertyTax, double hoaFee, double insurance) {
        
        double interestPercent = interestRate / 100;
        double loanTermMonthly = loanTerm * 12;
        double monthlyInterest = interestPercent / 12;
        double downPaymentPercent = downPayment / 100;
        double propertyPercent = propertyTax / 100;
        double hoaPercent = hoaFee / 100;
        double insurancePercent = insurance / 100;
        double closingCostPercent = 0.03;
        double maintenanceRate = 0.015;

        double monthlyIncome = annualIncome / 12.0;
        double maxHousingPayment = 0.28 * monthlyIncome;
        double maxTotalDebt = 0.36 * monthlyIncome;
        double adjustedMaxPITI = maxTotalDebt - monthlyDebtPayments;
        double allowedPITI = Math.min(maxHousingPayment, adjustedMaxPITI);


        double homePrice = allowedPITI / (0.28 - hoaPercent / 12.0 - insurancePercent / 12.0 - propertyPercent / 12.0);

        
        double monthlyHOA = homePrice * hoaPercent / 12;
        double monthlyPropertyTaxes = homePrice * propertyPercent / 12;
        double monthlyInsurance = homePrice * insurancePercent / 12;

        double principalAndInterest = allowedPITI - (monthlyHOA + monthlyPropertyTaxes + monthlyInsurance);


        double loanAmount;
        if (monthlyInterest == 0) {
            loanAmount = principalAndInterest * loanTermMonthly;
        } else {
            loanAmount = principalAndInterest /
                        (monthlyInterest * Math.pow(1 + monthlyInterest, loanTermMonthly) /
                        (Math.pow(1 + monthlyInterest, loanTermMonthly) - 1));
        }

        
        double downPaymentAmount = homePrice * downPaymentPercent;
        double totalHomePrice = homePrice;
        double closingCost = homePrice * closingCostPercent;
        double totalClosingCost = downPaymentAmount + closingCost;


        double frontEndDTI = (allowedPITI / monthlyIncome) * 100.0;
        double backEndDTI = ((allowedPITI + monthlyDebtPayments) / monthlyIncome) * 100.0;


        double monthlyMortgagePayment;
        if (monthlyInterest == 0) {
            monthlyMortgagePayment = loanAmount / loanTermMonthly;
        } else {
        ...
        double annualPropertyTax = homePrice * propertyPercent;
        double annualHOAFee = homePrice * hoaPercent;
        double annualInsuranceCost = homePrice * insurancePercent;
        double annualMaintenanceCost = homePrice * maintenanceRate;

        
        double totalMonthlyCost = monthlyMortgagePayment + (annualPropertyTax / 12) + monthlyHOA + (annualInsuranceCost / 12) + (annualMaintenanceCost / 12);
        
        return new double[] {loanAmount, totalHomePrice, downPaymentAmount, closingCost, frontEndDTI, backEndDTI, totalClosingCost, monthlyMortgagePayment, annualPropertyTax, annualHOAFee, annualInsuranceCost, annualMaintenanceCost, totalMonthlyCost};
    }
}
